...
PRODA AS (SELECT DISTINCT P.PROCODIGO FROM PRODU P
...
CLICODIGO,
 TBP.TBPCODIGO TBPCODIGO_COMB,
   PROCODIGOA PROCODIGO ,
    CCINDICEPROA2 DESCTO_A_COMB,
    ROUND(PRCA.PREPCOVENDA2*(1-CCINDICEPROA2*1.00/100),2) TOTAL_A_COMB,
      CCPCOVENDAPROA2 VALOR_A_COMB,
       PROCODIGOB ,
         CCINDICEPROB2 DESCTO_B_COMB,
          ROUND(PRCB.PREPCOVENDA2*(1-CCINDICEPROB2*1.00/100),2) TOTAL_B_COMB,
            CCPCOVENDAPROB2 VALOR_B_COMB,
             ROUND(PRCA.PREPCOVENDA2*(1-CCINDICEPROA2*1.00/100),2)*2+ROUND(PRCB.PREPCOVENDA2*(1-CCINDICEPROB2*1.00/100),2) TOTAL_COMB



FROM TBPCOMBPROPRO TBP
              INNER JOIN PRODA PA ON PA.PROCODIGO=TBP.PROCODIGOA
